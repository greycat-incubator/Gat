...
class SearchResult {
    ...
    protected Classification classify(CharSequence key, EStruct nodeFound, int charsMatched, int charsMatchedInNodeFound) {
        if (charsMatched == key.length()) {
            if (charsMatchedInNodeFound == ((String) nodeFound.get(RadixTreeStruct.NODE_ADDITION)).length()) {
                return Classification.EXACT_MATCH;
            } else if (charsMatchedInNodeFound < ((String) nodeFound.get(RadixTreeStruct.NODE_ADDITION)).length()) {
